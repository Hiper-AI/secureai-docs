...
- **NHIs** — API keys and service accounts (revocable — see [NHI Inventory](/zh/discovery/nhi-inventory))
...
After the first sync, open [Cloud Sensors](/zh/discovery/cloud-sensors) to see discovered models/assistants, and [NHI Inventory](/zh/discovery/nhi-inventory) to see keys and service accounts. Per-provider **Insights** show usage, cost, and per-user spend.

## Revocation

OpenAI API keys are **revocable** from [NHI Inventory](/zh/discovery/nhi-inventory): SecureAI can disable a key at the source. (For service accounts, OpenAI identities are monitor-only.)
