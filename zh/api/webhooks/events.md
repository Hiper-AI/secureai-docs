...
When creating or updating a [webhook endpoint](/zh/api/webhooks/overview), you subscribe it to one or more event types. Use `"*"` to receive every event.
...
| `api:model_failover` | A [redundancy](/zh/api/redundancy) chain failed over from one model to another. |
...
- [Webhooks Overview](/zh/api/webhooks/overview) — delivery format, signature verification, reliability.
