...
The `receipt` object is the signed payload emitted by the gateway. See [SMLTP Security](/zh/security/smltp) for how receipts fit into the transparency and audit model.
...
- Receipts are held in the gateway's in-memory store for a limited time. For long-term proof, rely on the **hash-chained audit log** — the authoritative, immutable record. See [Immutable Logs](/zh/security/immutable-logs).
...
- [SMLTP Security](/zh/security/smltp)
- [Immutable Logs](/zh/security/immutable-logs)
