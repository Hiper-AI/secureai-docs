...
| `api_key.billing_mode` | `user-completions` or `usage-by-model` — see [Billing Modes](/zh/api/billing-modes). |
...
- To preview whether a *specific* request would pass its quota (without spending), use [Policy Check](/zh/api/policy-check) and read `checks.quota`.

## Related

- [Billing Modes](/zh/api/billing-modes)
- [Authentication Overview](/zh/api/auth/overview)
- [Policy Check](/zh/api/policy-check)
