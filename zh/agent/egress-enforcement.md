...
Egress enforcement controls what an endpoint is allowed to send **out** to AI providers and MCP servers. It's configured per [policy](/zh/agent/policies-and-groups) and enforced locally by the agent.
...
- Start in **off** (or a **monitor**-mode policy) to build an accurate picture of what the endpoint actually calls — visible in the device drawer and in [AI Discovery](/zh/discovery/overview).
...
Egress modes are also bundled into the [protection presets](/zh/agent/policies-and-groups#protection-presets) (Passive / Normal / Aggressive), so you can set a whole-posture level in one click and fine-tune from there.

<Warning>
`kill`, `block`, and `lockdown` actively interrupt endpoint traffic. Validate a change with **dry-run simulate** and staged **rollout rings** ([Policies & Groups](/zh/agent/policies-and-groups#safe-rollouts)) before applying it fleet-wide.
</Warning>

## Related

- [Policies & Groups](/zh/agent/policies-and-groups)
- [Transparent Proxy](/zh/agent/transparent-proxy) — route allowed AI traffic through the gateway instead of blocking it.
