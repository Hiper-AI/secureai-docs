...
| `api_key.billing_mode` | `user-completions` or `usage-by-model` — see [Billing Modes](/api/billing-modes). |
...
- To preview whether a *specific* request would pass its quota (without spending), use [Policy Check](/api/policy-check) and read `checks.quota`.

## Related

- [Billing Modes](/api/billing-modes)
- [Authentication Overview](/api/auth/overview)
- [Policy Check](/api/policy-check)
