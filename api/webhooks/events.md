...
When creating or updating a [webhook endpoint](/api/webhooks/overview), you subscribe it to one or more event types. Use `"*"` to receive every event.
...
| `api:model_failover` | A [redundancy](/api/redundancy) chain failed over from one model to another. |
...
- [Webhooks Overview](/api/webhooks/overview) — delivery format, signature verification, reliability.
