...
Create-time inputs: `url`, `description`, `events[]` (see [Events](/api/webhooks/events)), and `enabled`. The signing secret (`whsec_...`) is shown **only** on create and rotate — store it securely; you cannot retrieve it again.
...
- [Webhook Events](/api/webhooks/events) — the full event catalog and payloads.
- [Redundancy & Failover](/api/redundancy) — source of `api:model_failover`.
- [Threat Defense](/threat-defense/overview) — source of the `promptshield:*` events.
