...
- **Agents, models, and deployments** running on the platform → shown in [Cloud Sensors](/discovery/cloud-sensors).
- **Non-Human Identities (NHIs)** — API keys, service accounts, and (where supported) BYOK keys → shown in [NHI Inventory](/discovery/nhi-inventory), with block/revoke controls.
...
Connector **setup** lives in **Admin → Integrations**. The **inventory** it produces appears in **Admin → Agent Registry**: discovered agents/models under [Cloud Sensors](/discovery/cloud-sensors), and discovered identities under [NHI Inventory](/discovery/nhi-inventory). You can re-run a sync from either place.
</Info>
...
|----------|-----------------|
| [OpenAI Platform](/integrations/cloud/openai-platform) | Organization admin API key |
| [Anthropic Console](/integrations/cloud/anthropic-console) | Admin API key (+ optional workspace / compliance keys) |
| [Azure AI Foundry](/integrations/cloud/azure-ai-foundry) | Entra service principal (tenant/client/secret) |
| [Google Vertex AI](/integrations/cloud/gcp-vertex-ai) | Service-account JSON |

## Related

- [Cloud Sensors](/discovery/cloud-sensors) — the discovered agent/model inventory.
- [NHI Inventory](/discovery/nhi-inventory) — the discovered identity inventory and revoke controls.
- [AI Discovery & Inventory Overview](/discovery/overview)
