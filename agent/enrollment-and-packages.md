...
| **Enrollment group** | The [group](/agent/policies-and-groups) a device is pinned to on first enroll. |
...
The package's **enrollment group** is a hard pin: a device that enrolls with the package is bound to that group on first contact. Group membership then drives which [policy](/agent/policies-and-groups) the device resolves.
...
- [Installing the Agent](/agent/installation)
- [Policies & Groups](/agent/policies-and-groups)
- [Self-update & anti-tamper](/agent/quarantine-and-fleet-ops#self-update--anti-tamper)
