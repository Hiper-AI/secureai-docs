...
- **Anti-tamper / uninstall key** — a [policy](/agent/policies-and-groups#tamper-protection--uninstall-key) can require an uninstall key so the agent can't be silently removed. Validation works even off-network (an offline salt/hash is carried to the endpoint), with a fleet-wide key as fallback; failed uninstall attempts are audited.
