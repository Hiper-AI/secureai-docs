...
The Agent Registry is about *logical* AI agents (a support chatbot, an internal copilot). It's different from the **OS Agents** fleet — the SecureAI software installed on endpoints, documented under [Endpoint Agent](/agent/overview).
</Info>
...
- Register chatbots for monitoring; a chatbot first seen through the [Prompt Shield API](/api/threat-defense/prompt-shield) is auto-registered here as `observed` for your review.

## Related

- [Endpoint Agent](/agent/overview) — the OS-level agent fleet (distinct from this).
- [NHI Inventory](/discovery/nhi-inventory) — the identities agents authenticate with.
- [Prompt Shield API](/api/threat-defense/prompt-shield) — auto-registers chatbots.
