...
- **Model redundancy** — a caller-defined failover chain (primary + up to 2 fallbacks). See [Redundancy & Failover](/pt/en/api/redundancy).
...
If you want to drop SecureAI into an existing OpenAI integration with **zero code changes**, use the [OpenAI-compatible endpoint](/pt/en/api/chat/openai-compatible) at `/api/external/v1/chat/completions` instead. This classic endpoint is the only one that supports RAG.
</Tip>
...
| `redundancy` | object | No | Chain-wide options: `{ timeout_ms, first_token_timeout_ms, on: [...] }`. See [Redundancy & Failover](/pt/en/api/redundancy). |
...
| `prompt_shield` | object | No | Per-call Prompt Shield control: `{ enabled?: boolean, policy?: string }`. See [Prompt Shield API](/pt/en/api/threat-defense/prompt-shield#per-call-control-on-completions). |
...
| `user_id` | string | No | MongoDB ObjectId of the user to bill this request to (admin-gated; see [Billing Modes](/pt/en/api/billing-modes)). |
...
- To validate a request against every policy **without** calling a model or spending points, use [Policy Check](/pt/en/api/policy-check).
- For failover chain semantics (triggers, timeouts, streaming behavior, exhaustion status codes), see [Redundancy & Failover](/pt/en/api/redundancy).
